...
double rcpplooei( double lambda, const Eigen::VectorXd& Lambda, 
                  ...
                  bool paral, Rcpp::Nullable<int> threads = R_NilValue)
{
                   ...
                   Rcpp::Nullable<int> threads = R_NilValue)
{
  try
  {
    ...
    svdeig ee = RcppbdSVD(eX, int(), int(), true, false);
...
res <- microbenchmark( looe2 <- LOOE(MD,YD,paral=TRUE, threads = 2),
                       looe4 <- LOOE(MD,YD,paral=TRUE),
                       times = 2L, unit = "s")
