                                      ...
                                      Rcpp::Nullable<int> threads  = R_NilValue)
{
  ...
  if( K == L)
  {
    ...
    create_HDF5_dataset( filename, strsubgroupOUT + "CrossProd_" + matA, M, M, "real");
    ...
    DataSet* datasetC = new DataSet(file->openDataSet(strsubgroupOUT + "CrossProd_" + matA));
    ...
    for (int ii = 0; ii < N; ii += hdf5_block)
    {
      ...
      for (int jj = 0; jj < M; jj += hdf5_block)
      {
        ...
        for(int kk = 0; kk < K; kk += hdf5_block)
        {
          ...
          Eigen::MatrixXd A = GetCurrentBlock_hdf5_Original( file, datasetA, jj, kk, 
                                                             std::min(hdf5_block,jsize),std::min(hdf5_block,ksize));
          
          Eigen::MatrixXd B = GetCurrentBlock_hdf5( file, datasetB, ii, kk, 
...
////          
  ...
  try{
    ...
    if(bparal == true)
    {
      ...
      hdf5_block_matrix_crossprod_hdf5(A, dsizeA, iblock_size, filename, strsubgroupIn, strsubgroupOut, 
...
res <- BigDataStatMeth::blockCrossprod_hdf5("BasicMatVect.hdf5", "INPUT","matA", block_size = 2)
...
res <- h5fdelay$OUTPUT$CrossProd_matA
res
