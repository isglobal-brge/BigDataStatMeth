 ...
 * - Dynamic OpenMP scheduling based on system type
 * - Performance monitoring and diagnostics
 * - Smart thread allocation respecting job allocations
 *
 * Key improvements:
 * - Respects job scheduler allocations (SLURM, PBS)
 * - Optimized OpenMP configuration without limiting threads
 * - Static scheduling for server environments
 * - Intelligent block sizes for memory efficiency
 * - System-aware pragma parameter selection
 *
 */
...
namespace BigDataStatMeth {
                                                             ...
                                                             int block_size, Rcpp::Nullable<int> threads  = R_NilValue)
    {
        ...
        try {
            ...
            if( A.cols() == B.rows())
            {
                // System-aware block size adjustment
                if(block_size > std::min( N, std::min(M,K)) )
                    block_size = std::min( N, std::min(M,K)); 
                
                // Apply system-specific block size optimizations
                if (sys_info.is_hpc) {
                    block_size = std::min(block_size, 512); // Optimized blocks for HPC
                } else if (sys_info.is_linux_server && sys_info.num_cores > 64) {
                    block_size = std::min(block_size, 768); // Medium blocks for large servers
                }
                ...
                // System-aware OpenMP pragma
                if (sys_info.is_hpc || sys_info.is_linux_server) {
                    // Optimized approach for HPC/Linux servers
                    #pragma omp parallel num_threads(optimal_threads) shared(A, B, C)
                    {
                    ...
                    // Standard approach for desktops
                    #pragma omp parallel num_threads(optimal_threads) shared(A, B, C)
                    {
    ...
    // Main multiplication function with intelligent system optimization
    inline void multiplication( BigDataStatMeth::hdf5Dataset* dsA, BigDataStatMeth::hdf5Dataset* dsB, BigDataStatMeth::hdf5Dataset* dsC,
                                       Rcpp::Nullable<bool> bparal, Rcpp::Nullable<int> hdf5_block, Rcpp::Nullable<int> threads  = R_NilValue) 
    {
        try {
            ...
            // System-aware block size determination
            if( hdf5_block.isNotNull()) {
                ...
                // Apply system-specific block size optimizations
                if (sys_info.is_hpc) {
                    ihdf5_block = std::min(ihdf5_block, (int)(MAXBLOCKSIZE/4)); // Smaller blocks for HPC
                } else if (sys_info.is_linux_server && sys_info.num_cores > 64) {
                    ihdf5_block = std::min(ihdf5_block, (int)(MAXBLOCKSIZE/3.5)); // Moderate blocks for large servers
                }
            }
            ...
            if( K == L )
            {
                ...
                if( dsC->getDatasetptr() != nullptr) {
                    ...
                    // System-aware parallel processing
                    if (sys_info.is_hpc || sys_info.is_linux_server) {
                        // Optimized approach for HPC/Linux servers
                        #pragma omp parallel num_threads(optimal_threads) shared(dsA, dsB, dsC, vstart, vsizetoRead)
                        {
                            #pragma omp for schedule(static) nowait
                            for (hsize_t ii = 0; ii < vstart.size(); ii++)
                            {
                        ...
                        // Standard approach for desktops (original code with dynamic scheduling)
                        #pragma omp parallel num_threads(optimal_threads) shared(dsA, dsB, dsC, vstart, vsizetoRead)
                        {
                            #pragma omp for schedule(dynamic) nowait
                            for (hsize_t ii = 0; ii < vstart.size(); ii++)
