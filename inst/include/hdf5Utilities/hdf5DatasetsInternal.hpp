            ...
            if(Rcpp::is<Rcpp::NumericMatrix>(DatasetValues) || Rcpp::is<Rcpp::IntegerMatrix>(DatasetValues) ||
               Rcpp::is<Rcpp::NumericVector>(DatasetValues) || Rcpp::is<Rcpp::IntegerVector>(DatasetValues) ) {
                ...
                if(vOffset[0] + hsCount[0] <= dimDataset[0] || vOffset[1] + hsCount[1] <= dimDataset[1]) {
                    ...
                    ::Rf_error( "It is not possible to write block in current position (writeDatasetBlock_internal)" );
                }
            } else {
                ::Rf_error( "Matrix data type not allowed (writeDatasetBlock_internal)" );
            }

        } catch(H5::FileIException& error) { // catch failure caused by the H5File operations
            close_dataset();
            close_dataset_file();
            ::Rf_error( "c++ exception writeDatasetBlock_internal (File IException)" );
        } catch(H5::DataSetIException& error) { // catch failure caused by the DataSet operations
            close_dataset();
            close_dataset_file();
            ::Rf_error( "c++ exception writeDatasetBlock_internal (DataSet IException)" );
        } catch(H5::GroupIException& error) { // catch failure caused by the Group operations
            close_dataset();
            close_dataset_file();
            ::Rf_error( "c++ exception writeDatasetBlock_internal (Group IException)" );
        } catch(H5::DataSpaceIException& error) { // catch failure caused by the DataSpace operations
            close_dataset();
            close_dataset_file();
            ::Rf_error( "c++ exception writeDatasetBlock_internal (DataSpace IException)" );
        } catch(H5::DataTypeIException& error) { // catch failure caused by the DataSpace operations
            close_dataset();
            close_dataset_file();
            ::Rf_error( "c++ exception writeDatasetBlock_internal (Data TypeIException)" );
        }
        return void();
    }
