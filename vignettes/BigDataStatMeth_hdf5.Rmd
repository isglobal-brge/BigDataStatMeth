...
Dealing with [hdf5 files](https://www.hdfgroup.org/solutions/hdf5/) requires: `HDF5Array`, `rhdf5`. The user can execute this code to install the required packages:

```{r install_required, eval=FALSE}
BiocManager::install(c("Matrix", "RcppEigen", "Spectra",
                       "beachmat", "DelayedArray",
                       "HDF5array", "rhdf5"))
```


Our package needs to be installed from source code. In such cases, a collection of software (e.g. C, C++, Fortran, ...) are required, mainly for Windows users. So, beThese programs can be installed using [Rtools](https://cran.r-project.org/bin/windows/Rtools/)
...
We have implemented the `Create_HDF5_matrix_file ()` function to create an hdf5 file with a group and a dataset in a single step. This function allows to create datasets from `DelayedMAtrix` objects using `beachmat` library as well as standard R matrices from `Matrix` objects.
...
Create_HDF5_matrix_file("robject.hdf5", A, "INPUT", "A")
```
...
The function `Add_HDF5_matrix ()` allows to add a dataset in a existing hdf5 file. We can create the dataset in any group, if group doesn't exists in file, the group is created before append the dataset. The object can be either an R object or a `DelayedArray` object.
...
Add_HDF5_matrix(B, "delayed.hdf5", "INPUT", "B")
```
...
Add_HDF5_matrix(B, "delayed.hdf5", "INPUT", "BTransposed", transp = TRUE);
```
...
You can easily change your text files to HDF5 using the function `Import_text_to_HDF5 ()`, to do that, we only have to define the input file with the `filename` parameter and the HDF5 destination dataset with parameters `outputfile` for the output file name, `outGroup` with the group to store the dataset and the dataset name with the parameter `outDataset`. It should be considered that:
...
Import_text_to_HDF5(filename = "colesterol.csv", 
                    sep=',', 
...
```{r crossprod }
res <- Crossprod_hdf5(filename = 'robject.hdf5', group="INPUT", A="A", outgroup = "RESULTS")
```
...
Add_HDF5_matrix(B, filename = 'robject.hdf5', group="INPUT", dataset = "B2")

# Get Crossprod with two matrix
res <- Crossprod_hdf5(filename = 'robject.hdf5', group="INPUT", A="A", 
...
```{r tcrossprod }
res <- tCrossprod_hdf5(filename = 'robject.hdf5', group="INPUT", A="A", outgroup = "RESULTS")
```
...
Add_HDF5_matrix(B, filename = 'robject.hdf5', group="INPUT", dataset = "B3")

# Get Crossprod with two matrix
res <- tCrossprod_hdf5(filename = 'robject.hdf5', group="INPUT", A="A", 
...
Add_HDF5_matrix(odata, "delayed.hdf5", "OMICS", "data")
...
Add_HDF5_matrix(geno.sim, "delayed.hdf5", "OMICS", "geno")
...
The implemented function to normalize data direct from hdf5 data file is `Normalize_hdf5`.
