...
We have implemented the `bdCreate_hdf5_matrix_file ()` function to create an hdf5 file with a group and a dataset in a single step. This function allows to create datasets from `DelayedMAtrix` objects using `beachmat` library as well as standard R matrices from `Matrix` objects.
...
The function `bdAdd_hdf5_matrix ()` allows to add a dataset in a existing hdf5 file. We can create the dataset in any group, if group doesn't exists in file, the group is created before append the dataset. The object can be either an R object or a `DelayedArray` object.
...
svdh5 <- bdSVD_hdf5("delayed.hdf5", "OMICS", "data", k = 2, q = 1)
