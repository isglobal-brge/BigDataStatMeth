...
The `Add_HDF5_matrix` allows to create a dataset in existing file. We can create the dataset in any group, if group doesn't exists in file, the group is created before append the dataset. The object could be an R object or a DelayedArray object.
...
The function `Add_HDF5_matrix` allows to create a dataset with a transposed matrix with the parameter `transpose = TRUE``, for example, we can add a dataset in hdf5 with the B transposed matrix with : 
...
About the delimiter parameter, the tabulation delimiter is taken in to account by default in function `Import_text_to_HDF5` but this can be changed easily with `sep` parameter.
...
Import_text_to_HDF5("colesterol.csv", ',', 
...
Import_text_to_HDF5("colesterol_new.csv", ',', 
                     outputfile = "colesterol_file.hdf5", outGroup = "COLESTEROLGROUP", outDataset = "COLESTEROLDATASET", 
...
Add_HDF5_matrix(B, filename = 'robjects.hdf5', group="INPUT", dataset = "B2")
...
Add_HDF5_matrix(B, filename = 'robjects.hdf5', group="INPUT", dataset = "B3")
