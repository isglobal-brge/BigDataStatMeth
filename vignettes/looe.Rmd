...
To work with big matrices `blockmult()` saves matrices in hdf5 file format in order to be able to operate with them in blocks and not overload the memory, by default are considered large matrices if if the number of rows or columns is greater than 5000, but it can be changed with `bigmatrix` argument.
...
If we work in memory results are returned to a matrix object, 
