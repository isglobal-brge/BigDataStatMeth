...
- `bdCrossprod()`  performs $$\left( C \right) ={ \left( A \right)  }^{ t }\left( A \right) $$

- `bdtCrossprod()` performs $$\left( C \right) = \left( A \right){ \left( A \right)  }^{ t } $$
...
- 'XtwX' or 'xtwx': `t(X) %*% diag(w) %*% X`
- 'XwXt' or 'xwxt': `X %*% diag(w) %*% t(X)`
- 'Xw' or 'xw': `X %*% diag(w)` 
- 'wX' or 'wx': `diag(w) %*% X`
