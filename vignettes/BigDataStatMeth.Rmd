...
| Split, reduce, and apply | `split()`, `split_dataset()`, `reduce()`, `hdf5_reduce()`, `apply_function()`, `hdf5_apply()` |
...
The arguments `nu` and `nv` control how many left and right singular
vectors are returned. Unlike `base::svd()`, however, they also change
*how* the decomposition is computed --- see the next section.
...
### When the decomposition is exact, and when it is not

`svd()` on an `HDF5Matrix` does not always return an exact decomposition,
and the result records which regime it is in. Two independent things
matter, and they are not equally important.

**The algorithm.** `method = "full"` reads the whole matrix into RAM and
runs an exact LAPACK SVD. `method = "blocks"` runs a hierarchical block
SVD entirely on disk. `method = "auto"` (the default) uses `"full"` while
`nrow * ncol` stays below `svd_auto_threshold()` and `"blocks"` at or
above it:
...
Crossing that boundary is, by itself, not much of a numerical event: at
full rank the block path agreed with LAPACK to relative errors around
`1e-15` across the whole spectrum in our tests, regardless of `k` and `q`.

**The requested rank.** This is the one to watch. Asking for fewer
singular triplets than `min(dim(x))` --- through `nu`, `nv`, or
`ncomponents`/`rank.` in `prcomp()` --- truncates *every local block SVD*
to that rank before the blocks are merged, and the truncation compounds
across levels. On a 1600 x 200 matrix with a decaying spectrum, relative
...
The error is not confined to the tail: at rank 10 even the *leading*
singular value was off by 1--3%. If you need a handful of accurate
components, compute a generously oversampled rank and discard the extra
ones --- computing 50 and keeping 10 was about 250 times more accurate in
the leading value than computing 10 directly.

Both switches are silent by default, so check them on the result:
...
In this small example `truncated` is `FALSE` even though only five singular
vectors were requested: the matrix is far below the boundary, so the exact
LAPACK path ran on the whole matrix and the reduction to five columns happened
afterwards, losslessly. The truncation only bites once the block algorithm is
in play.

`svd()` also emits a `message()` whenever it enters either approximate
regime; wrap the call in `suppressMessages()` in scripted pipelines that
have already accounted for it.
...
# Dataset operations: split, reduce, and apply

## Split

`split()` divides an `HDF5Matrix` into equal-sized blocks stored as
separate datasets in the same HDF5 file. Use `n_blocks` to specify the
number of blocks, or `block_size` to specify the maximum number of rows
(or columns, if `bycols = TRUE`) per block.

```{r split-example, eval=FALSE}
...
`split_dataset()` is an equivalent with a cleaner signature that omits
the `f` and `drop` parameters inherited from `base::split`. Both
produce identical results.

Note: `BigDataStatMeth::split()` produces an error because `split` is
a `base` generic — this is the same behaviour as
`BigDataStatMeth::cor()` or `BigDataStatMeth::svd()`. The correct call
is simply `split(X, n_blocks = 3)` after loading the package.

## Reduce

`reduce()` accumulates all datasets in the same HDF5 group as `x` into
a single dataset using `"+"` or `"-"`. This is useful after a split
operation or when partial results from parallel computation are stored
as separate datasets in a group.

```{r reduce-example, eval=FALSE}
fn <- tempfile(fileext = ".h5")
...
`hdf5_reduce(filename, group, ...)` provides the same operation when
you only have the file path and group name, without needing an open
`HDF5Matrix` object.

## Apply function

`apply_function()` applies a predefined algebraic or statistical
operation to a named set of datasets within the HDF5 group of `x`.

**This is not equivalent to `base::apply()`.** It does not apply an
arbitrary R function row- or column-wise. Instead, it dispatches
built-in C++ operations — QR decomposition, cross-products, Cholesky,
and others — to a batch of datasets in the file. Support for
row/column-wise application of arbitrary R functions
(`apply.HDF5Matrix`) is planned for a future release.

```{r apply-function-example, eval=FALSE}
fn <- tempfile(fileext = ".h5")
...
`hdf5_apply(filename, group, datasets, func, outgroup, ...)` provides
the same operation when you only have the file path, without an open
object.
